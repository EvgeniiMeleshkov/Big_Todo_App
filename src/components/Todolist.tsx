...
export function Todolist(props: PropsType) {
    ...
    const editTaskTitle = (newTitle: string, taskId: string) => {
    ...
    return <div>
        <span className='titleSpan'>
            <h3><EditableSpan title={props.title} callBack={(newTitle)=>editTodoTitle(newTitle)}/></h3>
            <button className='delButton' onClick={()=>props.deleteTodo(props.todolistID)}>X</button>
        </span>
                ...
                props.tasks.map(t => {
                    ...
                    return <li key={t.id} className={t.isDone ? "is-done" : ""}>
                        <input type="checkbox"
                               ...
                               checked={t.isDone}/>
                        <EditableSpan callBack={(newTitle)=>editTaskTitle(t.id, newTitle)} title={t.title}/>
